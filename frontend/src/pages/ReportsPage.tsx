...
export default function ReportsPage() {
  ...
  const reintChartData = reintRows.map(r => ({ name: r.status ?? 'Unknown', value: Number(r.count ?? 0) }));
  ...
  const compChartData = Object.entries(safehouseMap).map(([name, v]) => ({ name, ...v }));

  // Annual report — find "Donation Summary" section for a bar chart
  const donationSummarySection = annual?.find(s => s.title.toLowerCase().includes('donation'));
  const donationSummaryRows: any[] = (donationSummarySection?.data as any[]) ?? [];

  // Annual services section (caring/healing/teaching counts)
  const servicesSection = annual?.find(s => s.title.toLowerCase().includes('service'));
    ...
    <div>
      ...
      <div className="container-fluid py-4">
        ...
        <div className="card mb-4">
          <div className="card-header fw-semibold">{trends?.title ?? 'Donation Trends'}</div>
            ...
            {trendChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={trendChartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <YAxis yAxisId="left" tick={{ fontSize: 12 }} tickFormatter={v => `₱${Number(v).toLocaleString()}`} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12 }} />
                  <Tooltip
                    formatter={(value: number, name: string) =>
                      name === 'total' ? [`₱${value.toLocaleString()}`, 'Total (PHP)'] : [value, 'Donations']
                    }
                  />
                  <Legend />
                  <Line yAxisId="left" type="monotone" dataKey="total" stroke="#2563eb" strokeWidth={2} dot={false} name="Total (PHP)" />
                  <Line yAxisId="right" type="monotone" dataKey="count" stroke="#16a34a" strokeWidth={2} dot={false} name="# Donations" />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="row g-4 mb-4">
          <div className="col-md-5">
            <div className="card h-100">
              <div className="card-header fw-semibold">{reintegration?.title ?? 'Reintegration Outcomes'}</div>
              <div className="card-body">
                <p className="text-muted small mb-3">{reintegration?.description}</p>
          ...
          <div className="col-md-7">
            <div className="card h-100">
              <div className="card-header fw-semibold">{comparison?.title ?? 'Safehouse Comparison'}</div>
              <div className="card-body">
                <p className="text-muted small mb-3">{comparison?.description}</p>
                {compChartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={compChartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                      <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                      <YAxis tick={{ fontSize: 12 }} />
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="residents" fill="#2563eb" name="Residents" radius={[4, 4, 0, 0]} />
                      <Bar dataKey="incidents" fill="#dc2626" name="Incidents" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="text-muted">No safehouse comparison data available.</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="card mb-4">
          <div className="card-header fw-semibold">Annual Accomplishment Report — {year}</div>
          <div className="card-body">
            {/* Donation by type bar chart if available */}
            {donationSummaryRows.length > 0 && (
              <div className="mb-4">
                <h6 className="fw-semibold mb-2">{donationSummarySection?.title}</h6>
                <p className="text-muted small mb-2">{donationSummarySection?.description}</p>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={donationSummaryRows} layout="vertical" margin={{ left: 20, right: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                    <XAxis type="number" tick={{ fontSize: 12 }} tickFormatter={v => `₱${Number(v).toLocaleString()}`} />
                    <YAxis type="category" dataKey="donationType" tick={{ fontSize: 12 }} width={120} />
            ...
            {servicesRows.length > 0 && (
              <div className="mb-4">
                ...
                <p className="text-muted small mb-2">{servicesSection?.description}</p>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={servicesRows} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
            ...
            {annual?.filter(s =>
              ...
              <div key={s.title} className="mb-4">
                <h6 className="fw-semibold" style={{ color: 'var(--brand-dark)' }}>{s.title}</h6>
                      ...
                      <thead className="table-light">
                        <tr>
                          {Object.keys((s.data as any[])[0]).map(k => (
                            <th key={k} className="text-capitalize">{k.replace(/([A-Z])/g, ' $1').trim()}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
